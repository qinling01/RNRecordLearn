...
//        KVNProgress.show(0.1)
        ...
        let progressView = ZZCircleProgress.init(frame: CGRect.init(origin: CGPoint.init(x: 150, y: 300), size: CGSize.init(width: 80, height: 80)))
        progressView.duration = 0
        progressView.showPoint = false
        progressView.strokeWidth = 3.0
        progressView.pathFillColor = UIColor.blue
        progressView.increaseFromLast = true
        progressView.progressLabel.font = UIFont.systemFont(ofSize: 15)
