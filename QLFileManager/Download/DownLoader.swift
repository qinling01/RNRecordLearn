...
class DownLoader: NSObject  {
    ...
    func downLoader(url : NSURL) {
        ...
        self.downLoadingPath = kTempPath + "QinL/" + fileName!
        self.downLoadedPath = kCachePath! + "QinL/" + fileName!
...
//        self.totalSize = String(resp.allHeaderFields["Content-Length"]).components(separatedBy: "/").last).CLongLong
        ...
        let string = resp.allHeaderFields["Content-Length"] ?? ""
        
        let stri : String = (string as AnyObject).components(separatedBy: "/").last!
                
        self.totalSize = CLongLong(stri) ?? 0
...
//            print(self.downLoadingPath,self.downLoadedPath)
            // 2. 取消本次请求
